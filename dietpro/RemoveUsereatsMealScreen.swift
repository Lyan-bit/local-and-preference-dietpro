...
struct RemoveUsereatsMealScreen: View { 
	@State var x :String = ""
  	@State var y :String = ""
  ...
  var body: some View {
    ...
    VStack(spacing: 20) {
      ...
      HStack(spacing: 20) {
        Button(action: { self.model.removeUsereatsMeal(x: x, y: y) } ) { Text("Remove") }
        Button(action: { self.model.cancelRemoveUsereatsMeal() } ) { Text("Cancel") }
     ...
     }.navigationTitle("removeUsereatsMeal")
    }
  }
}
